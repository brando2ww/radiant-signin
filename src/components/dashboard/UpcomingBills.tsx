...
export const UpcomingBills = ({
  ...
  return <Card className="animate-fade-in" style={{
    animationDelay: '300ms'
  }}>
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <span className="flex items-center gap-2">Contas Próximas</span>
      ...
      <CardContent>
        <div className="space-y-3">
          {bills.length === 0 ? <div className="text-center py-8 text-muted-foreground">
              <AlertCircle className="h-8 w-8 mx-auto mb-2 opacity-50" />
              <p className="text-sm">Nenhuma conta próxima</p>
            </div> : bills.slice(0, 4).map(bill => {
          const daysUntil = getDaysUntilDue(bill.dueDate);
          return <div key={bill.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50 hover:bg-muted transition-colors">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <p className="font-medium text-sm">{bill.title}</p>
                      <Badge variant={getBadgeVariant(daysUntil)} className="text-xs">
                        {daysUntil === 0 ? 'Hoje' : daysUntil < 0 ? `${Math.abs(daysUntil)}d atrasado` : `${daysUntil}d`}
                      </Badge>
                    </div>
                    <p className="text-xs text-muted-foreground">
                      {format(bill.dueDate, "dd 'de' MMMM", {
                  locale: ptBR
                })}
                    </p>
                  </div>
