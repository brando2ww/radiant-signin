...
export function ProductRecipeManager({ productId, productPrice }: ProductRecipeManagerProps) {
  const { recipes, isLoading, addIngredient, updateQuantity, removeIngredient, calculateCMV, calculateMargin } = usePDVRecipes(productId);
  ...
  const handleUpdateQuantity = (id: string, newQuantity: string) => {
    ...
    if (numQuantity > 0) {
      updateQuantity({ id, quantity: numQuantity });
    }
  };
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Insumos e custos que compõem este produto
          </CardDescription>
        </CardHeader>
        <CardContent>
          {recipes.length === 0 ? (
                    ...
                    <TableRow key={recipe.id}>
                      ...
                      <TableCell>
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => removeIngredient({ id: recipe.id, productId })}
                        >
