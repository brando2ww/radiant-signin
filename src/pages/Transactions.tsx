      ...
      <div className="w-full max-w-7xl mx-auto">
        ...
        {isMobile ? <div className="sticky top-0 z-10 bg-background/95 backdrop-blur-sm border-b border-border/40 px-4 py-3">
            <div className="flex items-center justify-between">
              ...
              <Button onClick={handleNewTransaction} size="icon" className="h-9 w-9 rounded-full">
                <Plus className="h-5 w-5" />
              </Button>
            </div>
          </div> : <ResponsivePageHeader title="Receitas e Despesas" description="Registre e acompanhe todas as transações do seu MEI" action={<Button onClick={handleNewTransaction} className="w-full md:w-auto">
        ...
        <div className={isMobile ? "px-4 py-4 space-y-4 w-full" : "space-y-6"}>
          ...
          {isMobile && <div className="flex items-center justify-between bg-card rounded-2xl p-3 shadow-sm overflow-hidden">
          ...
          {/* Filters */}
          <TransactionFilters filters={filters} onFilterChange={setFilters} transactionCount={transactions.length} />

          {/* Transaction List */}
          <TransactionList transactions={transactions} isLoading={isLoading} onEdit={handleEdit} onDelete={handleDeleteClick} />
        </div>

        {/* Create/Edit Dialog */}
        <TransactionDialog open={dialogOpen} onOpenChange={setDialogOpen} onSubmit={handleCreateOrUpdate} transaction={selectedTransaction} isSubmitting={isCreating || isUpdating} />

        {/* Delete Confirmation Dialog */}
        <DeleteConfirmDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen} onConfirm={handleDeleteConfirm} transaction={selectedTransaction} />
      </div>
    </AppLayout>;
}