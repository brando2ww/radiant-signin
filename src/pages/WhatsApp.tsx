...
export default function WhatsApp() {
  ...
  if (isLoading) {
    return <AppLayout className="flex-1 flex items-center justify-center p-4 md:p-6 lg:p-8">
        <motion.div animate={{
        rotate: 360
      }} transition={{
        duration: 1,
  ...
  return <AppLayout className="space-y-6 p-4 md:p-6 lg:p-8">
      <ResponsivePageHeader title="Verificação de WhatsApp" description="Verifique seu número de WhatsApp para receber notificações" className="mt-4 md:mt-6 lg:mt-8" />
      ...
      <div className="max-w-md mx-auto space-y-6 mt-8 md:mt-12">
        ...
        <div className="flex items-center justify-center gap-2">
          ...
          return <div key={step.id} className="flex items-center">
              ...
              backgroundColor: isCompleted ? "hsl(var(--primary))" : isCurrent ? "hsl(var(--primary) / 0.2)" : "hsl(var(--muted))"
            }} className={cn("flex items-center justify-center w-10 h-10 rounded-full transition-colors", isCompleted && "text-primary-foreground", isCurrent && "text-primary ring-2 ring-primary ring-offset-2 ring-offset-background", !isCompleted && !isCurrent && "text-muted-foreground")}>
                  <Icon className="h-5 w-5" />
                </motion.div>
                
                {index < steps.length - 1 && <div className={cn("w-8 h-0.5 mx-1 transition-colors", currentStep > step.id ? "bg-primary" : "bg-muted")} />}
              </div>;
        })}
        </div>
        ...
        <div className="flex justify-between px-2">
          {steps.map(step => <span key={step.id} className={cn("text-xs font-medium transition-colors", currentStep >= step.id ? "text-primary" : "text-muted-foreground")}>
              {step.label}
            </span>)}
        </div>
        ...
        <AnimatePresence mode="wait">
          {isVerified ?
        // Verified state
        <motion.div key="verified" initial={{
          opacity: 0,
          y: 20,
          scale: 0.95
        }} animate={{
              ...
              <Card className="border-green-500/50 bg-green-500/5 overflow-hidden">
                ...
                <CardContent className="text-center space-y-6 relative">
                  ...
                  <div className="space-y-2">
                    <Button variant="default" onClick={() => navigate('/settings')} className="w-full bg-green-600 hover:bg-green-700">
          ...
          duration: 0.3
              ...
              <Card>
                ...
                <CardContent className="space-y-6">
                  {/* Timer Progress Bar */}
                  {timeLeft !== null && timeLeft > 0 && <div className="space-y-2">
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-muted-foreground flex items-center gap-1">
                          <Clock className="h-4 w-4" />
                          Tempo restante
                        </span>
                        <motion.span animate={timeLeft <= 30 ? {
                    scale: [1, 1.1, 1]
                  } : {}} transition={{
                    duration: 0.5,
                    repeat: timeLeft <= 30 ? Infinity : 0
                  }} className={cn("font-mono font-bold", timeLeft <= 30 && "text-destructive", timeLeft > 30 && timeLeft <= 60 && "text-yellow-500", timeLeft > 60 && "text-primary")}>
                          {formatTimeLeft(timeLeft)}
                        </motion.span>
                      </div>
                      <Progress value={timeLeft / TOTAL_TIME * 100} className={cn("h-2 transition-all", getTimerColor())} />
                    </div>}

                  {timeLeft === 0 && <motion.div initial={{
                opacity: 0,
                y: -10
              }} animate={{
                opacity: 1,
                y: 0
                  ...
                  <div className="flex justify-center">
                    <InputOTP maxLength={6} value={code} onChange={handleCodeChange} disabled={isVerifying || timeLeft === 0} autoFocus>
                      <InputOTPGroup className="gap-2 md:gap-3">
                        {[0, 1, 2, 3, 4, 5].map(index => <InputOTPSlot key={index} index={index} className="w-10 h-12 md:w-12 md:h-14 text-lg md:text-xl rounded-lg border-2 transition-all focus-within:ring-2 focus-within:ring-primary" />)}
                      </InputOTPGroup>
                    </InputOTP>
                  </div>

                  {/* Auto-submit indicator */}
                  {code.length > 0 && code.length < 6 && <p className="text-center text-xs text-muted-foreground">
                      {6 - code.length} dígitos restantes
                    </p>}

                  {isVerifying && <motion.div initial={{
                opacity: 0
              }} animate={{
                opacity: 1
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-center gap-2 text-sm">
                      ...
                      <Button variant="link" size="sm" onClick={handleResendCode} disabled={resendCooldown > 0 || isSending} className="p-0 h-auto">
                        {resendCooldown > 0 ? <span className="text-muted-foreground">
                ...
                <CardContent className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Input id="phone" type="tel" placeholder="(11) 99999-9999" value={phone} onChange={handlePhoneChange} disabled={isSending} className="flex-1 h-11" autoFocus />
                    </div>
                  ...
                  <Button onClick={handleSendCode} disabled={phone.replace(/\D/g, "").length < 10 || isSending} className="w-full h-11 bg-green-600 hover:bg-green-700">
                    {isSending ? <>
