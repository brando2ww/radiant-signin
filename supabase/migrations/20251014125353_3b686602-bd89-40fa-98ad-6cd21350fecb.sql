-- Inserir cartões de crédito e transações de exemplo para Eduardo Brando
DO $$
DECLARE
  v_user_id uuid := 'a0f02206-4422-49b2-a7fb-91ac73c60562'; -- Eduardo Brando
  v_card_nubank uuid;
  v_card_inter uuid;
  v_card_c6 uuid;
BEGIN
  ...
  INSERT INTO credit_cards (
    ...
    true
  ) RETURNING id INTO v_card_c6;

  -- Inserir transações vinculadas ao Nubank
  ...
  -- Inserir transações vinculadas ao Inter
  INSERT INTO transactions (user_id, type, category, amount, description, transaction_date, payment_method, credit_card_id, is_recurring)
  VALUES
  ...
  -- Inserir transações vinculadas ao C6 Bank
  INSERT INTO transactions (user_id, type, category, amount, description, transaction_date, payment_method, credit_card_id, is_recurring)
  VALUES
    (v_user_id, 'expense', 'Compras', 300.00, 'Americanas - Eletrodomésticos', CURRENT_DATE - INTERVAL '4 days', 'credit_card', v_card_c6, false),
    (v_user_id, 'expense', 'Alimentação', 95.00, 'Supermercado', CURRENT_DATE - INTERVAL '9 days', 'credit_card', v_card_c6, false);

  -- Inserir transações gerais (não vinculadas a cartões)
